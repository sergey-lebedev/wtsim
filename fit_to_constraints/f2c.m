function result = f2c(shiftings, lb, ub, residual, siz, ratio)
...
acolcnt(a(siz-1)) = acolcnt(a(siz-1)) + 1;
...
acolcnt(b(siz-1)) = acolcnt(b(siz-1)) + 1;
...
acolcnt(c(siz-1)) = acolcnt(c(siz-1)) + 1;
...
acolcnt(d(siz-1)) = acolcnt(d(siz-1)) + 1;
%acolidx(idx) = fun(siz-1);
%acolnzs(idx) = 6;
%idx=idx+1;
acolidx(idx) = fun(siz-1);
...
for i=1:siz-1
	lbound(a(i)) = shiftings(i)-ub(i);
        ubound(a(i)) = shiftings(i)-lb(i);
        lbound(b(i)) = -big;
        ubound(b(i)) = big;
        lbound(c(i)) = -big;
        ubound(c(i)) = big;
...
rhs(m) = shiftings(siz)-ub(siz);
ubound(m+n) = (shiftings(siz)-lb(siz)) - (shiftings(siz)-ub(siz));
lbound(m+n) = 0;
...
result(siz)=X(a(siz-1))+X(b(siz-1))+X(c(siz-1))+X(d(siz-1));
residual=result;
size(shiftings);
size(result);
result=shiftings-result';
